...
```{r}
library(tidyr)
plot.MSE <- function(simnum,pi,mu,R,nu,k,path,n, leg=TRUE)
{
  mse <- data.frame()
  for(i.pi in pi)
  {
...
```{r}
plot.false.pos.neg <- function(simnum,pi,mu,R,nu,k,path,n, xaxis=FALSE)
{
  ...
  plt <- ggplot(data=df,aes(x=k,y=value,fill=type)) + 
    ...
    ggtitle(paste0("n = ",n))+ ylab("")+xlab(xlabel)+
    theme(
        plot.title = element_text(hjust=0.5, size=rel(2)),
