...
function main()
    ...
    νs = [10,12,16] #5#10
    seed = 2358
    sampsizes = [50,100,200]
    simtypes = ["additive_random","interaction_random","redundant_random",
    "additive_phylo","interaction_phylo","redundant_phylo"]
    t = 30
    tmot = true
    Random.seed!(seed)
    rng = Xoshiro(seed)
    ...
    idx=1
    tot_idx=length(νs)*length(μₛs)*length(πₛs)*length(ks)*length(sampsizes)*length(simtypes)
                ...
                for k in ks
                    for sampsize in sampsizes
                        for typ in simtypes
                            ...
                            println(stderr,"mu=",μₛ," pi=",πₛ," k=",k," simtype=",typ," sampsize=",sampsize, " R=", R, " ν=",ν)
                            idx = idx+1
